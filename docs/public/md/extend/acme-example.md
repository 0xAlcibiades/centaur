...
# Keep the 1Password service-account token separate from the broad infra Secret.
kubectl create secret generic centaur-iron-proxy-source-auth \
  --namespace centaur-system \
  --from-file=OP_SERVICE_ACCOUNT_TOKEN=/run/secrets/centaur-op-service-account-token
```
