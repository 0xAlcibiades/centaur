...
An existing principal must carry the exact workflow identity labels; api-rs
fails closed instead of reusing a principal left by a differently named
workflow. Scoped `ctx.agent_turn(...)` calls cannot choose `thread_key`, so a
workflow cannot attach its grants to an unrelated session.
