...
path. This is opt-in for deployments that need Codex subscriptions or Claude
Code subscription/card auth. In normal API-key mode, keep using the harness
...
Claude Code local auth uses these optional keys in `centaur-harness-auth`:
...
The Kubernetes sandbox backend scopes auth payloads by engine. Do not put
these payloads in `centaur-infra-env`; the API pod imports that Secret with
`envFrom`.
