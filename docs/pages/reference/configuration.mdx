...
| `CODEX_AUTH_MODE` | `sandbox.codexAuthMode`. | Codex auth flow: `api_key` (default, hits `api.openai.com`), `access_token` (hits `chatgpt.com` with a Console-managed broker credential), or `autorotate` (a Console-pinned Autorotate overlay). In `autorotate` mode the shared proxy role injects no Codex headers, so a missing or invalid pin has no fallback credential. Helm sets it on api-rs before the matching proxy fragment is registered, then propagates it to sandboxes; do not override it through `sandbox.extraEnv`. See [Codex Auth Modes](/deploying-in-production#codex-auth-modes). |
